...
func (c *Client) PlanOperation(ctx context.Context, request OperationRequest) (OperationPlan, error) {
	...
	plan := OperationPlan{newOperationID(), request.Action, target(request), request.Reason, current, desired, impact, deployment.ResourceVersion, true, true, false, time.Now().UTC()}
...
func (c *Client) validateOperation(request OperationRequest) error {
	...
	if request.Action != OperationRestart && request.Action != OperationScale {
...
func (c *Client) applyOperation(ctx context.Context, request OperationRequest, resourceVersion string, dryRun bool) error {
	...
	payload, _ := json.Marshal(map[string]any{"spec": map[string]any{"template": map[string]any{"metadata": map[string]any{"annotations": map[string]string{"kubevista.io/restart-request": request.PlanID, "kubectl.kubernetes.io/restartedAt": time.Now().UTC().Format(time.RFC3339)}}}}})
...
func (c *Client) consumeOperationPlan(request OperationRequest) error {
	c.plans.mu.Lock()
	defer c.plans.mu.Unlock()
