...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      <header className="topbar"><div className="breadcrumbs"><span>Clusters</span><b>/</b><strong>kubevista-dev</strong><b>/</b><span>{activeName}</span></div><div className="topbar-actions"><span className={`connection-pill ${requestState}`}><span/>{isDemoMode?'Validated snapshot':requestState==='connected'?'Live stream':requestState==='loading'?'Connecting':'API unavailable'}</span><button className="icon-button" onClick={()=>void refresh()} disabled={refreshing} aria-label="Refresh current view"><Icon name="refresh"/></button></div></header>
      <div className="workspace">{isDemoMode&&<div className="demo-banner"><div><strong>Portfolio demo</strong><span>Representative data backed by a validated AWS EKS deployment. The paid cluster is intentionally offline.</span></div><a href="https://github.com/andy98w/Kubernetes-Dashboard" target="_blank" rel="noreferrer">Architecture &amp; evidence ↗</a></div>}{requestState==='stale'&&<div className="notice" role="status"><Icon name="warning"/><div><strong>Live inventory is unavailable</strong><span>{error}. Showing the last successful response where available.</span></div><button onClick={()=>void refresh()}>Retry connection</button></div>}{requestState==='loading'||!validData(view,cache[view])?<LoadingPage/>:<Page view={view} data={cache[view]}/>}</div>
    </main>
  </div>
}
