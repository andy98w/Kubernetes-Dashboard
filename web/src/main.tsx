...
import './styles.css'
import {demoData, demoWorkloadDetail, isDemoMode} from './demo-data'
...
type WorkloadDetail = { workload:Workload; strategy:string; selector:Record<string,string>; labels:Record<string,string>; images:Array<{name:string;image:string}>; pods:PodDetail[]; services:Service[]; policies:Policy[]; events:ClusterEvent[]; observedAt:string }
...
function App() {
  ...
  const refresh = useCallback(async (signal?:AbortSignal) => {
    setRefreshing(true)
    try {
      const value=isDemoMode?demoData(view):await fetchJSON(`/api/v1/${view === 'overview' ? 'summary' : view}`,signal)
      setCache(current=>({...current,[view]:value})); setRequestState('connected'); setError(null)
    } catch(caught) {
  ...
  useEffect(()=>{
    ...
    const stream=new EventSource('/api/v1/stream'); let debounce=0
    stream.addEventListener('cluster-update',()=>{window.clearTimeout(debounce);debounce=window.setTimeout(()=>void refresh(),350)})
    return()=>{window.clearTimeout(debounce);stream.close()}
  },[refresh])
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="environment-switcher"><span className={`status-dot ${requestState}`}/><div><small>Environment</small><strong>{isDemoMode?'Demo data':'Development'}</strong></div><span className="chevron">⌄</span></div>
      ...
      <div className="sidebar-footer"><button className={view==='settings'?'active':''} onClick={()=>navigate('settings')}><Icon name="settings"/><span>Settings</span></button><div className="version-row"><span>KV</span><div><strong>Dashboard build</strong><small>v0.2.0</small></div></div></div>
    </aside>
    <main>
      <header className="topbar"><div className="breadcrumbs"><span>Clusters</span><b>/</b><strong>kubevista-dev</strong><b>/</b><span>{activeName}</span></div><div className="topbar-actions"><span className={`connection-pill ${requestState}`}><span/>{isDemoMode?'Demo data':requestState==='connected'?'Live stream':requestState==='loading'?'Connecting':'API unavailable'}</span><button className="icon-button" onClick={()=>void refresh()} disabled={refreshing} aria-label="Refresh current view"><Icon name="refresh"/></button></div></header>
      <div className="workspace">{isDemoMode&&<div className="demo-banner"><div><strong>Demo data</strong><span>Based on the August 31 EKS test run. That cluster has since been shut down.</span></div><a href="https://github.com/andy98w/Kubernetes-Dashboard" target="_blank" rel="noreferrer">View deployment notes ↗</a></div>}{requestState==='stale'&&<div className="notice" role="status"><Icon name="warning"/><div><strong>Live inventory is unavailable</strong><span>{error}. Showing the last successful response where available.</span></div><button onClick={()=>void refresh()}>Retry connection</button></div>}{requestState==='loading'||!validData(view,cache[view])?<LoadingPage/>:<Page view={view} data={cache[view]}/>}</div>
    </main>
  </div>
}
...
function WorkloadsPage({data}:{data:Workloads}) {
  ...
  return <><PageHeading eyebrow="Live controllers" title="Workloads" description="Replica health across Deployments, StatefulSets, DaemonSets, Jobs, and CronJobs." observedAt={data.observedAt}/><section className="metric-grid three"><Metric label="Controllers" value={String(data.items.length)} detail="Observed resources" icon="workloads" status="neutral"/><Metric label="Healthy" value={String(healthy)} detail={`${data.items.length?Math.round(healthy/data.items.length*100):0}% available`} icon="check" status="healthy"/><Metric label="Needs attention" value={String(data.items.length-healthy)} detail="Progressing or unavailable" icon="warning" status={healthy===data.items.length?'healthy':'warning'}/></section><section className="panel data-panel"><PanelHeader title="Controller inventory" description="Select a workload to inspect pods, images, exposure, policy, and events" meta={`${filtered.length} shown`}/><Filters><input aria-label="Search workloads" placeholder="Search workload or kind" value={query} onChange={e=>setQuery(e.target.value)}/><select aria-label="Filter namespace" value={namespace} onChange={e=>setNamespace(e.target.value)}>{namespaces.map(ns=><option key={ns} value={ns}>{ns==='all'?'All namespaces':ns}</option>)}</select></Filters><DataTable headers={['Workload','Namespace','Kind','Ready','Status','Age']}>{filtered.map(item=><tr className="interactive-row" tabIndex={0} onClick={()=>void open(item)} onKeyDown={e=>{if(e.key==='Enter')void open(item)}} key={`${item.namespace}/${item.kind}/${item.name}`}><td><strong>{item.name}</strong></td><td><code>{item.namespace}</code></td><td>{item.kind}</td><td>{item.ready}/{item.desired}</td><td><Status value={item.status}/></td><td>{age(item.createdAt)} <span className="row-arrow">→</span></td></tr>)}</DataTable>{!filtered.length&&<Empty message="No workloads match the current filters."/>}</section>{selected&&<WorkloadDrawer item={selected} detail={detail} error={detailError} onClose={close}/>}</>
}
...
function IncidentsPage({data}:{data:Incidents}) {
  ...
  return <><PageHeading eyebrow="Event review" title="Incidents" description="Warning events grouped by affected resource and time." observedAt={data.observedAt}/><section className="metric-grid three"><Metric label="Incidents" value={String(data.items.length)} detail="Shown in this view" icon="warning" status="neutral"/><Metric label="Active" value={String(active)} detail="Still requiring attention" icon="warning" status={active?'critical':'healthy'}/><Metric label="Resolved" value={String(data.items.length-active)} detail="No longer active" icon="check" status="healthy"/></section><section className="incident-list">{data.items.map(item=><article className="panel incident-card" key={item.id}><div className="incident-summary"><div><Status value={item.severity}/><Status value={item.status}/></div><h2>{item.title}</h2><p>{item.summary}</p><code>{item.namespace} · {item.resource}</code></div><div className="timeline">{item.evidence.map((e,index)=><div className="timeline-item" key={`${e.source}-${index}`}><i/><div><small>{relative(e.at)} · {e.source}</small><strong>{e.detail}</strong></div></div>)}</div></article>)}</section>{!data.items.length&&<div className="panel"><Empty message="No warning events are currently correlated into incidents."/></div>}</>
}

function WorkloadDrawer({item,detail,error,onClose}:{item:Workload;detail:WorkloadDetail|null;error:string|null;onClose:()=>void}) {
