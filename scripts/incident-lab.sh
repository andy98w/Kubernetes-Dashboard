...
case "$scenario" in
  ...
  crashloop) "${k[@]}" patch deployment probe-failure --type=json -p='[{"op":"add","path":"/spec/template/spec/containers/0/command","value":["/bin/sh","-c","echo injected-crash >&2; exit 1"]}]' ;;
  ...
  # Make the allocator PID 1 so an OOM kill terminates the container, not just a shell child.
  oom) "${k[@]}" patch deployment probe-failure --type=json -p='[{"op":"add","path":"/spec/template/spec/containers/0/command","value":["/bin/sh","-c","exec head -c 134217728 /dev/zero > /dev/shm/fill"]},{"op":"add","path":"/spec/template/spec/volumes","value":[{"name":"memory","emptyDir":{"medium":"Memory"}}]},{"op":"add","path":"/spec/template/spec/containers/0/volumeMounts","value":[{"name":"memory","mountPath":"/dev/shm"}]}]' ;;
